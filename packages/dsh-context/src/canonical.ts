...
function assertJsonValue(value: unknown, path: string): asserts value is JsonValue {
  ...
  if (Array.isArray(value)) {
    for (let index = 0; index < value.length; index += 1) {
      if (!(index in value)) throw new TypeError(`${path}[${index}] must not be a sparse array entry`)
      assertJsonValue(value[index], `${path}[${index}]`)
    }
    return
  }
  if (typeof value !== 'object' || value === null) throw new TypeError(`${path} is not a JSON value`)
  const prototype = Object.getPrototypeOf(value)
  if (prototype !== Object.prototype && prototype !== null) throw new TypeError(`${path} must be a JSON object`)
...
export function sha256Utf8(value: string): string {
  ...
  for (let index = 0; index < 8; index += 1) padded[lengthOffset + index] = Math.floor(bitLength / 2 ** (56 - index * 8)) & 0xff
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      ...
      const smallSigma0 = rotateRight(prior, 7) ^ rotateRight(prior, 18) ^ (prior >>> 3)
