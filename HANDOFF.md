...
Continue `docs/superpowers/plans/2026-08-27-dsh-v0.1-foundation.md` with Task 2: validate the official `openai-codex` route through its opt-in smoke path, then add the v0.1 types and orchestration behavior task by task. Track an upstream fix for `node-addon-require-builtin` on Nix Node so the local `dsh-web` compatibility wrapper can eventually return to the standard launch path.
