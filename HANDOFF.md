...
Continue `docs/superpowers/plans/2026-08-27-dsh-v0.1-foundation.md` with Task 4: add the deterministic budget-admission service and its focused tests. Actual Cordis Loader composition, prompt/tool/event registration, subagent execution, and verification-process execution remain later tasks. Track an upstream fix for `node-addon-require-builtin` on Nix Node so the local `dsh-web` compatibility wrapper can eventually return to the standard launch path.
