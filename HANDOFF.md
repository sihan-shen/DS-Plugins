...
Validate the shortlisted plugins and current DSH extension points, then turn the v0.1 milestone into an implementation plan. Track an upstream fix for `node-addon-require-builtin` on Nix Node so the local `dsh-web` compatibility wrapper can eventually return to the standard launch path.
